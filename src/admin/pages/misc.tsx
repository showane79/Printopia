...
import { MediaStatusNotice } from "../components/MediaStatusNotice";
 ...
 *
 * Storage is optional. When it is unavailable this page still renders: existing
 * images remain listed and manageable, and the upload box is replaced by a
 * notice that explains why and offers a one-click re-check.
 */
export function Media() {
  ...
  return (
    <div>
      ...
      {/* Uploading is only offered when the server confirms storage accepts it. */}
      {status && !status.available ? (
        <MediaStatusNotice status={status} onResolved={(next) => { setStatus(next); if (next.available) void reload(); }} />
      ) : (
      ...
      {media.length === 0 ? (
        <div className="card p-10 text-center text-sm text-muted">
          {status && !status.available
            ? "هنوز تصویری ذخیره نشده است. می‌توانید بدون تصویر هم نوشته و محصول بسازید."
