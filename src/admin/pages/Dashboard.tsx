...
import { serverRepository, listAdminProducts } from "../services/repository";
...
import type { AdminPost, AdminProduct } from "../types";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    let alive = true;
    Promise.all([
      serverRepository.listPosts().catch(() => [] as AdminPost[]),
      listAdminProducts().catch(() => [] as AdminProduct[]),
    ]).then(([p, pr]) => {
      if (!alive) return;
      setPosts(p);
      setProducts(pr);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {stats.map((s) => (
          <div key={s.label} className="card p-5">
            <p className="text-3xl font-extrabold text-gradient">{loading ? "…" : toPersianDigits(s.value)}</p>
      ...
      <div className="card mt-6 p-5">
        ...
        {loading ? (
          <div className="p-8 text-center text-sm text-muted" aria-busy="true">
