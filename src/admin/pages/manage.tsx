...
import type { AdminNavItem, AdminRedirect } from "../types";

/* ----------------------------- Navigation ----------------------------- */
export function Navigation() {
  const { pushToast } = useStore();
  const [items, setItems] = useState<AdminNavItem[]>(() => listNav());
...
export function Appearance() {
  ...
  function set<K extends keyof typeof a>(k: K, v: (typeof a)[K]) {
    setA((p) => ({ ...p, [k]: v }));
  }
  function save() {
    saveAppearance(a);
...
export function Redirects() {
  const { pushToast } = useStore();
  const [items, setItems] = useState<AdminRedirect[]>(() => listRedirects());
...
export function ProductCategories() {
  const { pushToast } = useStore();
  const [items, setItems] = useState(() => localRepository.listCategories());
  function save() {
    localRepository.saveCategories(items);
...
export function SiteSettings() {
  const { pushToast } = useStore();
  const [s, setS] = useState(() => localRepository.getSettings());
  function set<K extends keyof typeof s>(k: K, v: (typeof s)[K]) {
    setS((p) => ({ ...p, [k]: v }));
  }
  function save() {
