...
interface ProvisionStep { key: string; label: string; ok: boolean; detail?: string }
...
export function Setup() {
  ...
  if (phase === "cloudflare-working") {
    const allOk = provisionSteps.length > 0 && provisionSteps.every((s) => s.ok);
    return (
      <Shell>
        <Card>
          ...
          <div className="space-y-2">
            {provisionSteps.map((step) => (
              <div key={step.key} className="flex items-start gap-3 rounded-xl border border-line bg-surface2 p-3">
                <span className={`mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full ${step.ok ? "bg-success/15 text-success" : "bg-danger/15 text-danger"}`}>
                  {step.ok ? <Check className="h-3 w-3" /> : "✕"}
                </span>
