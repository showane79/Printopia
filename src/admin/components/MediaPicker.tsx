 ...
 *
 * Image storage is optional. If it is not switched on yet, the picker explains
 * that in place — with a re-check button — instead of showing an empty grid or
 * an error, because the editor around it works fine without images.
 */

import { useEffect, useState } from "react";
import { Modal } from "../../components/Modal";

import { getMediaLibrary, MediaError, type MediaStatus } from "../services/media";
import { MediaStatusNotice } from "./MediaStatusNotice";
...
export function MediaPicker({ open, onClose, onSelect }: Props) {
  ...
  useEffect(() => {
    ...
    setError("");
    getMediaLibrary()
      .then((lib) => { if (alive) { setItems(lib.items); setStatus(lib.status); } })
      .catch((e) => { if (alive) setError(e instanceof MediaError ? e.message : "بارگذاری رسانه‌ها ناموفق بود."); })
      .finally(() => { if (alive) setLoading(false); });
    return () => { alive = false; };
  }, [open]);

  /** After a successful retry the library is refetched so the grid fills in. */
  function onResolved(next: MediaStatus) {
    setStatus(next);
    if (next.available) {
      getMediaLibrary()
        .then((lib) => { setItems(lib.items); setStatus(lib.status); })
        .catch(() => {});
    }
  }
    ...
    <Modal open={open} onClose={onClose} title="انتخاب تصویر" size="lg">
      <div className="max-h-[70vh] overflow-auto p-4">
        {loading ? (
          ...
          <MediaStatusNotice status={status!} onResolved={onResolved} />
        ) : items.length === 0 ? (
