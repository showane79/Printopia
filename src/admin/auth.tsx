...
/** How the Worker is reaching D1: fast binding, REST fallback, or not at all. */
export type DbMode = "binding" | "rest" | "none";
...
export interface SetupStatus {
  ...
  dbMode: DbMode; // which data path is live
  cloudflareConnected: boolean; // provisioning has already run
}
...
const OFFLINE: SetupStatus = {
  loading: false, serverAvailable: false, dbState: "unknown",
  setupComplete: false, dbMode: "none", cloudflareConnected: false,
};
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      const data = (await res.json()) as {
        dbState?: DbState; setupComplete?: boolean; authed?: boolean;
        dbMode?: DbMode; cloudflareConnected?: boolean;
      };
      setStatus({
        loading: false,
