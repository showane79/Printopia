...
export default function Blog() {
  ...
  useEffect(() => {
    ...
    fetch("/api/posts")
      ...
      .catch(() => { if (!cancelled) { setError("بارگذاری مقاله‌ها ناموفق بود."); setLoading(false); } });
  ...
  if (error) return <div className="container-x py-16 text-center text-muted">{error}</div>;
