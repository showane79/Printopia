      ...
      <Reveal>
        <Link to={`/blog/${feature.slug}`} className="group card mt-8 grid overflow-hidden md:grid-cols-2">
          <div className="relative aspect-[16/10] overflow-hidden md:aspect-auto">
            {feature.image && <img {...imgFallback} src={feature.image} alt={feature.title} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />}
          </div>
          ...
          <Reveal key={a.slug} delay={i * 60}>
            <Link to={`/blog/${a.slug}`} className="group card flex h-full flex-col overflow-hidden transition-all duration-300 hover:-translate-y-1.5 hover:border-accent/40">
              <div className="aspect-[16/10] overflow-hidden">
                {a.image && <img {...imgFallback} src={a.image} alt={a.title} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />}
              </div>
