...
import {
  ...
  getStore, toMediaDto, type MediaRow, type R2BucketLike,
...
async function provision(token: string): Promise<Response> {
  ...
  // 4b. Create/reuse the R2 bucket for media. Creating a bucket IS automatable;
  //     what Cloudflare does not let us automate is attaching a custom domain.
  //     We avoid needing one by serving objects through the Worker (/media/*),
  //     so the bucket stays private and no dashboard step is required.
  let bucketReady = false;
  const mkBucket = await cfFetch(token, `/accounts/${accountId}/r2/buckets`, {
    method: "POST", body: JSON.stringify({ name: R2_NAME }),
  });
  if (mkBucket.res.ok) {
    bucketReady = true;
    steps.push({ key: "r2", label: "فضای ذخیرهٔ تصویر (R2)", ok: true, detail: "باکت تازه ساخته شد" });
  } else {
    // Already exists → fine. Anything else is reported but not fatal, because
    // text content must still be able to finish setting up.
    const head = await cfFetch(token, `/accounts/${accountId}/r2/buckets/${R2_NAME}`);
    bucketReady = head.res.ok;
    steps.push({
      key: "r2", label: "فضای ذخیرهٔ تصویر (R2)", ok: bucketReady,
      detail: bucketReady ? "باکت موجود استفاده شد"
        : (mkBucket.res.status === 403
          ? "توکن دسترسی «Workers R2 Storage: Edit» را ندارد. بدون آن آپلود تصویر کار نمی‌کند."
          : mkBucket.error),
    });
  }
...
async function handleApi(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    if (path === "/api/admin/media" && method === "GET") {
      ...
      return json({ ok: true, media: rows.map((r) => toMediaDto(r, base)), storage: getStore(env, R2_NAME)?.mode ?? "none" }, { headers: NO_STORE });
    }

    if (isUpload) {
      const store = getStore(env, R2_NAME);
      if (!store) return fail("R2_NOT_READY", "فضای ذخیرهٔ تصاویر (R2) هنوز آماده نیست. در «تنظیمات → اتصال Cloudflare» راه‌اندازی را دوباره اجرا کنید.", 503);
