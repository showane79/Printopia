 ...
 *
 * Serves the built SPA from static assets and exposes a small, secure admin
 * auth API at /api/admin/*. The admin password hash and an auto-generated
 * session-signing secret are stored in D1 — so the owner NEVER has to create
 * ADMIN_PASSWORD_HASH or SESSION_SECRET manually.
 *
 * Storage: a single D1 database bound as `DB` (see wrangler.jsonc). If `DB` is
 * not bound yet, the API reports dbOk:false and the setup wizard guides the
 * owner to bind it. Nothing here breaks the public site.
 */
...
// ---- Crypto: PBKDF2 password hashing (matches client + is server-side) ---
...
function sessionCookieHeader(value: string, maxAge: number): string {
  const secure = "Secure"; // admin runs on HTTPS in production
...
async function setupComplete(env: Env): Promise<boolean> {
  return (await getAdminHash(env)) !== null;
}
...
async function handleApi(request: Request, env: Env): Promise<Response> {
  ...
  // Lightweight CSRF check: state-changing requests must send JSON.
  ...
  try {
    // STATUS — non-sensitive health + setup/auth state.
    if (path === "/api/admin/status" && request.method === "GET") {
      await ensureSchema(env);
      const dbOk = !!env.DB;
      const complete = dbOk && (await setupComplete(env));
      let authed = false;
      if (complete) {
    ...
    if (path === "/api/admin/setup" && request.method === "POST") {
      ...
      // Auto-generate the session secret server-side — no manual SESSION_SECRET needed.
    ...
    if (path === "/api/admin/login" && request.method === "POST") {
      if (!env.DB) return json({ message: "پایگاه‌داده متصل نیست." }, { status: 503 });
      await ensureSchema(env);
      if (!(await setupComplete(env))) {
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // Everything else: serve the SPA (with single-page-application fallback).
