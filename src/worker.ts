...
import {
  MEDIA_MAX_BYTES, MEDIA_ACCEPTED, sniffMime, imageSize, buildKey, safeKey,
  getStore, toMediaDto, probeMedia, clearMediaProbe, isR2NotEnabled,
  mediaReasonMessage, type MediaCapability, type MediaRow, type R2BucketLike,
} from "./media";
...
interface Env {
  ...
  /** R2 bucket binding for CMS media. Falls back to the REST API when absent. */
  MEDIA?: R2BucketLike;
...
const SCHEMA: string[] = [
  ...
  // Media metadata. The bytes live in R2; only the key + descriptive fields
  // live here, so D1 never stores binary blobs.
  `CREATE TABLE IF NOT EXISTS media (
    id TEXT PRIMARY KEY, object_key TEXT UNIQUE NOT NULL, original_filename TEXT DEFAULT '',
    mime_type TEXT NOT NULL, size_bytes INTEGER NOT NULL DEFAULT 0,
    width INTEGER DEFAULT 0, height INTEGER DEFAULT 0, alt_text TEXT DEFAULT '',
    created_at TEXT NOT NULL, created_by TEXT DEFAULT 'admin'
  );`,
...
// ---------------------------------------------------------------------------
/**
 * One line in the wizard's progress list. `optional` marks a step whose failure
 * must not fail the setup — the wizard shows it as "skipped", not as an error.
 */
interface Step { key: string; label: string; ok: boolean; detail?: string; optional?: boolean }
...
/**
 * OPTIONAL step: create or reuse the media bucket.
 *
 * Media is a bonus, not a prerequisite. If the account has R2 switched off
 * (code 10042), lacks the R2 permission, or R2 is simply unreachable, this
 * reports `ready: false` with a plain-Persian explanation and setup continues.
 * It never throws and never aborts provisioning.
 */
async function setupMedia(token: string, accountId: string): Promise<{ ready: boolean; step: Step }> {
  const label = "فضای ذخیرهٔ تصویر (اختیاری)";
  // `optional: true` is what keeps a missing bucket from reading as a failed setup.
  const off = (detail: string): { ready: boolean; step: Step } =>
    ({ ready: false, step: { key: "r2", label, ok: false, optional: true, detail } });


  try {
    const mk = await cfFetch(token, `/accounts/${accountId}/r2/buckets`, {
      method: "POST", body: JSON.stringify({ name: R2_NAME }),
    });
    if (mk.res.ok) return { ready: true, step: { key: "r2", label, ok: true, detail: "آماده شد" } };

    // The account has never turned R2 on. Expected, and explicitly not an error.
    if (isR2NotEnabled(mk.body, mk.res.status)) return off(mediaReasonMessage("r2_not_enabled"));

    // Any other failure may just mean "already exists" — confirm with a lookup.
    const head = await cfFetch(token, `/accounts/${accountId}/r2/buckets/${R2_NAME}`);
    if (head.res.ok) return { ready: true, step: { key: "r2", label, ok: true, detail: "فضای موجود استفاده شد" } };
    if (isR2NotEnabled(head.body, head.res.status)) return off(mediaReasonMessage("r2_not_enabled"));
    if (mk.res.status === 403) return off(mediaReasonMessage("no_permission"));
    return off(mediaReasonMessage("unreachable"));
  } catch (e) {
    console.warn("[printopia] R2 setup skipped:", (e as Error)?.message);
    return off(mediaReasonMessage("unreachable"));
  }
}
 ...
 *
 * Only the steps that the CMS cannot live without are fatal. Media is optional
 * and reported separately, so an account without R2 still finishes setup.
 */
...
async function provision(token: string): Promise<Response> {
  ...
  // 4b. OPTIONAL: media storage. Deliberately isolated from core setup — an
  //     account with R2 switched off (code 10042) must still finish setup and
  //     run the whole site. Never returns early, never rolls anything back.
  const media = await setupMedia(token, accountId);
  steps.push(media.step);
  const bucketReady = media.ready;
  ...
  const patch = await cfFetch(token, `/accounts/${accountId}/workers/scripts/${SCRIPT_NAME}/script-settings`, {
    method: "PATCH", body: JSON.stringify({ bindings: [
      ...
      ...(bucketReady ? [{ type: "r2_bucket", name: "MEDIA", bucket_name: R2_NAME }] : []),
    ] }),
  });
  steps.push({ key: "binding", label: "اتصال سریع پایگاه‌داده و R2", ok: patch.res.ok, detail: patch.res.ok ? undefined : patch.error });

  return json({
    ok: true, steps, accountId, databaseId, bucket: bucketReady ? R2_NAME : null,
...
async function handleApi(request: Request, env: Env): Promise<Response> {
  ...
  // Uploads are the one route that legitimately carries multipart/form-data.
  const isUpload = path === "/api/admin/media/upload" && method === "POST";
  const hasBody = (method === "POST" || method === "PUT" || method === "PATCH") && !isUpload;
  ...
  try {
    ...
    // ---- ADMIN: media ------------------------------------------------------
    // Bytes go to R2; only metadata is written to D1. Nothing touches the browser.
    // Every route here treats storage as optional: when it is off the CMS keeps
    // working and the UI is told exactly why, in Persian.

    // Capability probe. `?refresh=1` re-checks immediately, which is what the
    // "بررسی دوباره" button uses after the owner enables R2 in the dashboard.
    if (path === "/api/admin/media/status" && method === "GET") {
      const force = url.searchParams.get("refresh") === "1";
      const cap = await probeMedia(env, R2_NAME, force);
      return json({ ok: true, ...cap, message: mediaReasonMessage(cap.reason) }, { headers: NO_STORE });
    }

    // Retry provisioning just the bucket, using the stored token. Lets the owner
    // switch R2 on later without re-pasting anything or re-running full setup.
    if (path === "/api/admin/media/retry" && method === "POST") {
      if (!env.CF_API_TOKEN || !env.CF_ACCOUNT_ID)
        return fail("MEDIA_NOT_CONFIGURED", mediaReasonMessage("not_configured"), 503);
      const r = await setupMedia(env.CF_API_TOKEN, env.CF_ACCOUNT_ID);
      clearMediaProbe(); // the world may have changed — never answer from cache
      const cap = await probeMedia(env, R2_NAME, true);
      return json({
        ok: r.ready && cap.available, ...cap,
        message: cap.available ? "ذخیره‌سازی تصویر فعال شد." : (r.step.detail || mediaReasonMessage(cap.reason)),
      }, { headers: NO_STORE });
    }

    if (path === "/api/admin/media" && method === "GET") {
      const rows = await db!.all<MediaRow>("SELECT * FROM media ORDER BY created_at DESC");
      const base = await getConfig(db!, "media_base_url");
      const cap = await probeMedia(env, R2_NAME);
      // The list is served even when storage is down: existing rows stay
      // visible and manageable, only new uploads are blocked.
      return json({
        ok: true, media: rows.map((r) => toMediaDto(r, base)),
        storage: cap.storage, mediaAvailable: cap.available,
        mediaReason: cap.reason, mediaMessage: mediaReasonMessage(cap.reason),
      }, { headers: NO_STORE });
    }

    if (isUpload) {
      const cap: MediaCapability = await probeMedia(env, R2_NAME);
      const store = cap.available ? getStore(env, R2_NAME) : null;
      if (!store) return fail(cap.code, mediaReasonMessage(cap.reason), 503, { mediaAvailable: false, mediaReason: cap.reason });


      let form: FormData;
      try { form = await request.formData(); }
      catch { return fail("BAD_UPLOAD", "فایل ارسال‌شده خوانده نشد. دوباره تلاش کنید.", 400); }

      const file = form.get("file");
      if (!file || typeof file === "string") return fail("FILE_REQUIRED", "فایلی انتخاب نشده است.", 400);
      const alt = String(form.get("alt") ?? "").trim();
      if (!alt) return fail("ALT_REQUIRED", "متن جایگزین (alt) تصویر الزامی است.", 400);

      const blob = file as unknown as { size: number; name?: string; arrayBuffer(): Promise<ArrayBuffer> };
      if (blob.size > MEDIA_MAX_BYTES)
        return fail("FILE_TOO_LARGE", `حجم فایل زیاد است. حداکثر ${Math.round(MEDIA_MAX_BYTES / 1024 / 1024)} مگابایت.`, 413);
      if (blob.size === 0) return fail("FILE_EMPTY", "فایل خالی است.", 400);

      const bytes = await blob.arrayBuffer();
      // Trust the bytes, not the declared Content-Type: this is what stops a
      // renamed script from being stored and later served as an image.
      const mime = sniffMime(new Uint8Array(bytes.slice(0, 32)));
      if (!mime || !MEDIA_ACCEPTED.includes(mime))
        return fail("UNSUPPORTED_TYPE", "فرمت پشتیبانی نمی‌شود. فقط JPG، PNG، WebP، AVIF یا GIF.", 415);

      const original = String(blob.name || "image").slice(0, 120);
      const key = buildKey(mime, original);
      const { width, height } = imageSize(new Uint8Array(bytes), mime);

      try {
        await store.put(key, bytes, mime, { alt: encodeURIComponent(alt) });
      } catch (e) {
        console.error("[printopia] R2 put failed:", (e as Error)?.message);
        return fail("UPLOAD_FAILED", "ذخیرهٔ تصویر در R2 ناموفق بود. توکن باید دسترسی «Workers R2 Storage: Edit» داشته باشد.", 502);
      }
      ...
      try {
        await db!.run(
          "INSERT INTO media (id,object_key,original_filename,mime_type,size_bytes,width,height,alt_text,created_at,created_by) VALUES(?,?,?,?,?,?,?,?,?,?)",
          [id, key, original, mime, blob.size, width, height, alt, now, "admin"]);
      } catch (e) {
        // Never leave an orphan object behind if the metadata write fails.
        await store.delete(key).catch(() => {});
        console.error("[printopia] media insert failed:", (e as Error)?.message);
        return fail("DB_INSERT_FAILED", "ذخیرهٔ اطلاعات تصویر ناموفق بود.", 500);
      }

      const base = await getConfig(db!, "media_base_url");
      return json({ ok: true, media: toMediaDto({ id, object_key: key, original_filename: original, mime_type: mime, size_bytes: blob.size, width, height, alt_text: alt, created_at: now }, base) }, { headers: NO_STORE });
    }

    if (path.startsWith("/api/admin/media/") && (method === "DELETE" || method === "PUT")) {
      const id = decodeURIComponent(path.slice("/api/admin/media/".length));
      const row = await db!.first<MediaRow>("SELECT * FROM media WHERE id=?", [id]);
      if (!row) return fail("NOT_FOUND", "تصویر پیدا نشد.", 404);
      const base = await getConfig(db!, "media_base_url");

      // Rename / re-caption: metadata only, so the URL stays stable.
      if (method === "PUT") {
        const alt = String(body.alt ?? "").trim();
        if (!alt) return fail("ALT_REQUIRED", "متن جایگزین (alt) تصویر الزامی است.", 400);
        await db!.run("UPDATE media SET alt_text=? WHERE id=?", [alt, id]);
        return json({ ok: true, media: toMediaDto({ ...row, alt_text: alt }, base) }, { headers: NO_STORE });
      }

      // Dependency-aware delete: refuse while the image is still referenced,
      // so no page is left pointing at a dead object. `?force=1` overrides.
      const url2 = new URL(request.url);
      const force = url2.searchParams.get("force") === "1";
      const stored = toMediaDto(row, base).url;
      const like = `%${row.object_key}%`;
      const usedBy: string[] = [];
      for (const r of await db!.all<{ title: string }>("SELECT title FROM posts WHERE cover LIKE ? OR body LIKE ?", [like, like])) usedBy.push(`نوشته: ${r.title}`);
      for (const r of await db!.all<{ title: string }>("SELECT title FROM products WHERE image LIKE ?", [like])) usedBy.push(`محصول: ${r.title}`);
      for (const r of await db!.all<{ key: string }>("SELECT key FROM app_config WHERE value LIKE ?", [like])) usedBy.push(`تنظیمات: ${r.key}`);
      if (usedBy.length && !force)
        return fail("MEDIA_IN_USE", `این تصویر در ${usedBy.length} جای دیگر استفاده شده است. ابتدا آن‌ها را تغییر دهید یا حذف اجباری را تأیید کنید.`, 409, { usedBy, url: stored });

      const store = getStore(env, R2_NAME);
      if (store) {
        try { await store.delete(row.object_key); }
        catch (e) {
          console.error("[printopia] R2 delete failed:", (e as Error)?.message);
          return fail("DELETE_FAILED", "حذف فایل از R2 ناموفق بود. دوباره تلاش کنید.", 502);
        }
      }
      // Only drop the row once the object is really gone — no dangling records.
      await db!.run("DELETE FROM media WHERE id=?", [id]);
      return json({ ok: true, usedBy }, { headers: NO_STORE });
    }
...
/**
 * PUBLIC MEDIA DELIVERY — GET /media/<object key>
 *
 * Why the Worker serves these instead of a public bucket:
 *   • no manual Cloudflare dashboard step (no public-access toggle, no r2.dev,
 *     no custom domain needed for the site to work)
 *   • same-origin URLs, so no CORS and no third-party host in the markup
 *   • keys embed a uuid, so responses are immutable and the CDN keeps them at
 *     the edge — a Worker invocation only happens on a cache miss
 * Swapping in an R2 custom domain later needs no data migration: set the
 * `media_base_url` config and `mediaUrl()` starts emitting absolute URLs.
 */
async function handleMedia(request: Request, env: Env): Promise<Response> {
  ...
  // Reject anything that is not shaped like a key we generated (blocks traversal).
  if (!key || !safeKey(key)) return new Response("Not found", { status: 404 });

  const store = getStore(env, R2_NAME);
  if (!store) return new Response("Media storage not configured", { status: 503 });

  const obj = await store.get(key).catch(() => null);
  ...
  // Conditional request support keeps repeat views free.
  if (obj.etag && request.headers.get("If-None-Match") === obj.etag)
